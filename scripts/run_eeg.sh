...
# causal-memory horizon gate; nothing runs after it.
set -euo pipefail
cd "$(dirname "$0")/.."
...
LOG=${QRC_EEG_LOG:-results/eeg/run_gate.log}
mkdir -p "$(dirname "$LOG")"
: >"$LOG"
...
run_step gate_baselines "$PYTHON_BIN" scripts/run_gate_baselines.py
...
run_step gate_report "$PYTHON_BIN" scripts/make_gate_report.py
run_step useful_horizon_v2 "$PYTHON_BIN" scripts/make_useful_horizon_v2.py
run_step update_results "$PYTHON_BIN" scripts/update_results_gate.py
run_step pytest "$PYTHON_BIN" -m pytest -q
echo "[$(date -Is)] pipeline_stages_complete" | tee -a "$LOG"
run_step verify_gate env QRC_EEG_LOG="$LOG" "$PYTHON_BIN" scripts/verify_gate.py
